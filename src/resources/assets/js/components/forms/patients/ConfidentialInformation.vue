...
<template>
  <div class="container confidential-info-form">
    <div class="patient-contact-info-container">
      <div class="section section-add-note">
        <form
          ...
          novalidate
        >
          <h3 class="text-center header-part header-part-first" v-if="!isStep">
            Authorization to Release Confidential Information to Professionals
          </h3>
          <div class="inp-group">
            <div class="row">
              <div class="col-sm-7 row-centered">
              ...
              <div class="col-sm-5 row-centered">
            ...
            <div class="row">
              <div class="col-xs-12">
                <p class="form-text">
                  I understand that my records contain information about my
                  therapy sessions and my mental health. I understand that all
                  my records are protected by state and federal laws that
                  require they are kept confidential and require my written
                  consent to disclose.
                </p>
                <p class="form-text">
                  I,
                  <input
                    ...
                    readonly
                  />
                  , hereby authorize Change Within Reach, Inc. staff to exchange
                  information with
                  <input
                    id="hereby_information_with"
                    type="text"
                    class="form-control empty-input xs-6-width"
                    v-model="form_data.hereby_information_with"
                    autocomplete="new-password"
                    :disabled="exchangeWith && exchangeWith.length > 0"
                    required
                  />
                  <span class="form-text">
                    for the sole purpose of care coordination and provision of
                    the highest quality of care.
                  </span>
                </p>
                <p class="form-text">
                  I understand that I have the right to revoke this release at
                  any time.
                </p>
                <p class="form-text">
                  I have been informed and understand this authorization to
                  release records and information, the nature of listed content
                  that I am willing to release, and the implications of their
                  release. This request is voluntary.
                </p>
                <p class="form-text">
                  I witnessed that the person understood the content of this
                  authorization and freely gave his or her consent, but was
                  physically unable to provide a signature.
                </p>
              </div>
            </div>
          </div>
                    ...
                    <div class="row">
                      ...
                      <div class="col-xs-2">
                        <button
                          type="button"
                          class="btn btn-danger pull-right"
                          @click.prevent="clearSignature('patient-signature18')"
                        >
                          Clear
                        </button>
                      </div>
          ...
          <div class="form-note-button-block text-right" v-if="!isStep">
...
export default {
  ...
  data() {
    return {
      ...
      required_fields: ["hereby_information_with"],
    };
  },
  ...
  methods: {
    validateForm() {
      ...
      if (!this.isStep) {
        if (this.form_data.years_old >= 15) {
          if (this.signature_is_empty && !this.isStep) {
            has_errors = true;
          ...
          if (this.signature18_is_empty && !this.isStep) {
            has_errors = true;
    ...
    fetchData() {
      if (this.isStep) {
        this.initSimpleFormData();
      } else {
        let id = this.$route.params.patient_id;
        this.$store.dispatch("getPatientById", { id: id }).then((error) => {
          if (error.response) {
            if (
    ...
    saveForm() {
      ...
      if (!this.signature18_is_empty) {
        let datapair1 = $("#patient-signature18").jSignature(
          "getData",
          "image"
        );
        this.form_data.signature18 =
          "data:" + datapair1[0] + "," + datapair1[1];
      }

      this.$store
        .dispatch("storePatientConfidentialInformation", {
          data: this.form_data,
        ...
        .then((response) => {
          ...
          if (response.status === 200 || response.status === 201) {
            this.$store.commit("appendAlertMessage", {
              key: "chooseForm",
              data: {
            ...
            this.$router.push("/patient-forms/" + this.patient.id);
          } else {
  ...
  watch: {
    ...
    form_data: {
      handler: function (val) {
        if (this.data) {
          for (let key in val) {
            if (key !== "signature" && key !== "signature18")
  ...
  mounted() {
    if (this.exchangeWith) {
      this.form_data.exchange_with = this.exchangeWith;
      this.form_data.hereby_information_with = this.exchangeWith.join(", ");
    }
    this.fetchData();
    window.setTimeout(() => {
      var tmp = this;
