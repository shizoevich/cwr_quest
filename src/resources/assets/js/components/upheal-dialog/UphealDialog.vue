        ...
        <div class="google-meet-wrapper" v-if="currentTab">
                    ...
                    <div class="sent-form-row">
                        ...
                        >
                            <el-checkbox
                                ...
                                :disabled="currentTab.slug === 'videoSessionAppointment' && !isInvitationSent"
                                @change="changeCheckVirtualData(sendPatientForm.send_via_email, 'email')"
                            />
                            <el-input
                                type="email"
                        ...
                        >
                            <el-checkbox
                                ...
                                @change="changeCheckVirtualData(sendPatientForm.send_via_secondary_email, 'secondary_email')"
                            />
                            <el-input
                                type="email"
                        ...
                        >
                            <el-checkbox
                                ...
                                @change="changeCheckVirtualData(sendPatientForm.send_via_sms, 'phone')"
                            />
                            <el-input
                                type="tel"
            ...
            <div class="google-meet-footer">
                <template v-if="currentTab.slug !== 'videoSessionAppointment'">
                    <el-button v-if="currentTab.slug !== 'patientForm'" @click="prevTab">Back</el-button>
                    <el-tooltip
                        class="item"
                        effect="dark"
                ...
                <template v-else>
                    <template v-if="isInvitationSent">
                        <el-button
                            type="primary"
                            @click="startVideoSession(videoSessionAppointmentData.patient.upheal_therapist_session_url)"
                        >
                            Join
                        </el-button>
                        <el-tooltip
...
<script>
import UphealDialogAppointments from "./UphealDialogAppointments";
import UphealDialogVideoSession from "./UphealDialogVideoSession";
import { eventBus } from "../../app";

export default {
    name: "UphealDialog",
    components: {
        UphealDialogAppointments,
        UphealDialogVideoSession,
    },
    props: {
        patient: {
            type: Object,
    ...
    computed: {
        ...
        validationSendForm() {
            if (this.sendPatientForm.send_via_email && !Boolean(this.sendPatientForm.email)) {
                return false;
            }

            if (this.sendPatientForm.send_via_secondary_email && !Boolean(this.sendPatientForm.secondary_email)) {
                return false;
            }

            if (this.sendPatientForm.send_via_sms && !Boolean(this.sendPatientForm.phone)) {
                return false;
            }

            return (
                (this.sendPatientForm.send_via_email && Boolean(this.sendPatientForm.email)) && (this.sendPatientForm.send_via_secondary_email && Boolean(this.sendPatientForm.secondary_email)) ||
                (this.sendPatientForm.send_via_email && Boolean(this.sendPatientForm.email)) ||
                (this.sendPatientForm.send_via_secondary_email && Boolean(this.sendPatientForm.secondary_email)) ||
                (this.sendPatientForm.send_via_sms && Boolean(this.sendPatientForm.phone)) ||
                (!this.selectedForms.length && this.currentTab && this.currentTab.slug === "patientForm")
            );
        },
        ...
        isInvitationSent() {
            return this.videoSessionAppointmentData
                && this.videoSessionAppointmentData.upheal_meet
                && this.videoSessionAppointmentData.upheal_meet.invitations
                && this.videoSessionAppointmentData.upheal_meet.invitations.length;
        }
    },
    methods: {
        initTabsData() {
            this.tabsData = [
                {
                    ...
                    description: "<p>Before sending the invitation to the Telehealth session, please send all required documents to the patient</p>",
                    active: false,
                },
                {
                    id: 1,
                    ...
                    description: `<p>Please provide an email address or phone number to invite <b>${this.fullPatientName}</b> to join you in a Telehealth session.</p>`,
                    active: false,
                },
                {
                    id: 2,
        ...
        initPatientFormData() {
            this.isLoading = true;
            this.$store.dispatch("getPatientFormsForModal", this.patient.id)
                .then(({ data }) => {
        ...
        updateAppointmentList() {
            this.isLoading = true;
            this.$store.dispatch("getTelehealthAppointments", this.patient.id)
                .then(({ data }) => {
        ...
        updateVideoSessionAppointmentData() {
            this.isLoading = true;
            this.$store.dispatch("getUphealAppointmentDialogData", this.videoSessionAppointmentId)
                .then(({ data }) => {
        ...
        changeTab(slug) {
            if (slug === 'videoSessionAppointment' && !this.isInvitationSent) {
                this.$set(this.sendPatientForm, 'send_via_email', true);
            ...
            eventBus.$on("patientFormsValid", (isPatientFormsValid) => {
                if (startHandle && isPatientFormsValid) {
                    this.$refs.sendPatientForm.validate((valid) => {
                        if (valid) {
                            this.$store.dispatch("sendPatientForms", payload)
                                .then(() => {
                                    this.isVideoSessionAppointment
                                        ? this.changeTab("videoSessionAppointment")
                                        : this.changeTab("appointmentList");
                ...
                .then(() => {
                    this.$refs.sendPatientForm.validate((valid) => {
                        if (valid) {
                            ...
                            this.$store.dispatch("storeVideoSession", payload)
                                .then(() => {
        ...
        sendInvite(appointment) {
            ...
            this.$store.dispatch("storeUphealVideoSession", payload)
                .then(({ data }) => {
                    this.startVideoSession(data.upheal_meeting.patient.upheal_therapist_session_url);
                    this.updateDialogData();
                })
                .catch((error) => {
                    this.isLoading = false;
