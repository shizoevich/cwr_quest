...
class CompletedInitialSurveyStatisticsReportHelper implements AbstractStatisticsReportHelper
{
    public static function getData(string $startDate, string $endDate): array
    {
        ...
        PatientComment::query()
            ...
            ->where('patient_comments.comment_type', PatientComment::INITIAL_SURVEY_COMMENT_TYPE)
            ...
            ->each(function ($comment) use (&$data) {
                $data[] = [
                    ...
                    'Service Rate' => $comment->metadata['service_rate'],
                    'Provider Rate' => $comment->metadata['provider_rate'],
