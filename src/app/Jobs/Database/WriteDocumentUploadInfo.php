...
class WriteDocumentUploadInfo implements ShouldQueue
{
    ...
    {
        $info = PatientDocumentUploadInfo::where(['patient_document_id' => $this->document->id])->first();
        if(!$info) {
            $info = new PatientDocumentUploadInfo;
        }
        $info->patient_document_id = $this->document->id;
