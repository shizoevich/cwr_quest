...
class UpdateSalaryDataWhenFeeScheduleChanged implements ShouldQueue
{
     ...
     */
    public function __construct(array $priceData, Carbon $dateFrom)
    {
    ...
    public function handle()
    {
        foreach ($this->priceData as $data) {
            $planIds = PatientInsurancePlan::query()
                ->where('id', $data['plan_id'])
                ->orWhere('parent_id', $data['plan_id'])
                ->pluck('id');
            PatientVisit::query()
                ->select([
                    'patient_visits.id',
                ...
                ->where('patient_visits.provider_tariff_plan_id', '=', $data['tariff_plan_id'])
                ->where('patient_visits.procedure_id', '=', $data['procedure_id'])
                ->whereIn('patient_visits.plan_id', $planIds)
    ...
    private function processingVisit($visit)
    {
        $visit->salary->each(function(Salary $salaryRecord) use ($visit) {
            ...
            if(in_array($salaryRecord->type, [Salary::TYPE_REGULAR_VISIT, Salary::TYPE_TELEHEALTH_VISIT])) {
                ...
                if($visit->is_overtime) {
                    $fee *= Salary::OVERTIME_VISITS_RATE;
                }
                ...
                $fee = $visit->fee_per_visit;
                $paidFee = $visit->fee_per_missing_pn;
                if($visit->is_overtime) {
                    $fee *= Salary::OVERTIME_VISITS_RATE;
                    $paidFee *= Salary::OVERTIME_VISITS_RATE;
                }
