...
class GenerateNewPatientsInsuranceStatisticsReport extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'statistics-report:new-patients-insurance-statistics';
    ...
    {
        $patientsFirstAppointmentsSql = 'SELECT patients_id as patient_id, MIN(time) as min_time FROM appointments WHERE deleted_at IS NULL GROUP BY patients_id';
        ...
        $insurancesData = Appointment::query()
            ...
            ->join(
                \DB::raw("($patientsFirstAppointmentsSql) as pfa"),
                function ($join) {
                    $join->on('appointments.patients_id', '=', 'pfa.patient_id');
                    $join->on('appointments.time', '=', 'pfa.min_time');
                }
            )
    ...
    private function generateInsuranceReport($insurancesData)
    {
        ...
        $fileName = 'new_patients_insurances_statistics_2024.xlsx';
