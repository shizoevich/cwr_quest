...
class ResaveInitialAssessmentDocuments extends Command
{
     ...
     * @var string
     */
    protected $signature = 'resave-documents:initial-assessments';
    ...
    public function handle()
    {
        ...
        $initialAssessment = AssessmentForm::where('document_name', 'CWR Initial Assessment')->first();
        if (empty($initialAssessment)) {
            return;
        }
        ...
        PatientElectronicDocument::query()
            ...
            ->where('patient_electronic_documents.document_type_id', '=', $initialAssessment->id)
            ...
            ->chunkById(100, function ($patientDocuments) {
                foreach ($patientDocuments as $patientDocument) {
                    $strToDump = 'Patient initial assessment id: ' . $patientDocument->id . ';';
                    dump($strToDump);
